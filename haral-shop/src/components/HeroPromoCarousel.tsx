...
export function HeroPromoCarousel({
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      setInternalIndex((i) => (i + 1) % items.length);
    }, SLIDE_MS);
  ...
  const slides = (
    <div
      className={`relative overflow-hidden ${embedded ? "h-full rounded-xl shadow-md" : "rounded-2xl shadow-md"}`}
    >
      <div
      ...
      >
        {items.map((banner) => (
          <article
            key={banner.id}
            className={`relative h-full min-h-44 w-full shrink-0 bg-gradient-to-br sm:min-h-48 lg:min-h-[280px] ${banner.gradient} ${banner.pattern}`}
          >
  ...
  if (embedded) {
    return <div className="h-full">{slides}</div>;
  }
